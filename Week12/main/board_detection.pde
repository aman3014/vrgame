...
class BoardDetection {
  ...
  PImage convolute(PImage img, float[][] kernel, float normFactor) {
    ...
    for (int i = 1; i < result.height - 1; ++i) {
      for (int j = 1; j < result.width - 1; ++j) {
        result.pixels[i * result.width + j] = (int) (dotProd(img, kernel, i, j) / normFactor);
      }
    }
    ...
    for (int i = 0; i < result.height; ++i) {
      for (int j = 0; j < result.width; ++j) {
        if (i == 0 || j == 0 || i == result.height - 1 || j == result.width - 1) {
          ...
          result.pixels[i * result.width + j] = color(dotProd(img, kernel, i, j) / normFactor);
        }
      }
    }
  ...
  float dotProd(PImage img, float[][] kernel, int row, int col) {
    float ans = 0;
